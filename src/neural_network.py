    ...
    def fit(self, X_train, y_train, X_val, y_val, epochs=1000, batch_size=1, patience=20, verbose=True):
        ...
        for epoch in range(epochs):
            ...
            if epoch % 100 == 0 and verbose:
                print(f"Epoch {epoch}, Loss: {train_loss_epoch:.4f}, Batch size: {batch_size}\nValidation Loss: {avg_val_loss:.4f}")
        

    def _update_lr_on_plateau(self, current_val_loss):
        ...
        if self.lr_wait >= 10:
            self.eta = max(self.eta * self.decay, 0.00001)
            self.lr_wait = 0
