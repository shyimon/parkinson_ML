...
def _monk3_test(learning_rate, seed, verbose=False): 
    if verbose:
    ...
    params = {
        ...
        'eta': learning_rate,
        'l2_lambda': 0.001,  # Regolarizzazione più forte per gestire rumore
        'momentum': 0.9,
        'algorithm': 'sgd',
...
def grid_search_lr(n_seeds_per_lr=5, learning_rates=None):
    ...
    for lr in learning_rates:
        ...
        for seed_idx in range(n_seeds_per_lr):
            ...
            results = _monk3_test(learning_rate=lr, seed=seed, verbose=False)
...
def plot_bias_variance_epochs(all_results, dataset_name='MONK-3', save_path='monk3_bias_variance_epochs.png'):
    ...
    max_len = max(len(curve[0]) for curve in all_curves) if all_curves else 120
...
if __name__ == "__main__": 
    try:
        print("="*70)
        print(" MONK-3 - BINARY CLASSIFICATION (with noise)")
        print("="*70)
        ...
        if ensemble_acc >= test_results['test_accuracy']:  
            ...
            print(f"\n📊 Grafico ensemble salvato:  monk3_performance_ENSEMBLE.png")
        ...
        print(f"\n NOTA:  MONK-3 contiene 5% di rumore nei dati di training!")
        print(f"   Accuracy ~93-97% è considerata eccellente per questo dataset.")
        ...
        print(f"  L2 Lambda:       {best_results['params']['l2_lambda']} (forte regolarizzazione)")
        ...
        print(f"\n File salvati:")
        print(f"  - monk3_bias_variance_epochs. png (bias-variance vs epoche)")
        print(f"  - monk3_performance. png (singolo modello)")
        if ensemble_acc >= test_results['test_accuracy']:
            print(f"  - monk3_performance_ENSEMBLE.png (ensemble) ← NUOVO!")
