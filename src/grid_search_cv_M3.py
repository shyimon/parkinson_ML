...
class GridSearchM3:
    ...
    def __init__(self, cv_folds=5, verbose=True, results_dir='grid_search_results_monk3'):
    ...
    def _local_refinement(self, X, y, best_params, radius=0.1, n_points=5):
        ...
        for param, value in best_params.items():
            ...
            if param == 'learning_rate':
                ...
                max_val = min(value * (1 + radius), 0.25)
            elif param == 'hidden_neurons':
                min_val = max(value - 2, 2)
                max_val = value + 2
            elif param == 'epochs':
                min_val = max(value - 75, 200)
                max_val = value + 75
            else:
                min_val = value * 0.9
                max_val = value * 1.1
    ...
    def _dichotomic_search(self, X, y, max_iteration=4, n_points=5):
        print("\n" + "="*60)
        print("RICERCA DICOTOMICA N-DIMENSIONALE - MONK3")
        print("="*60)
        ...
        for iteration in range(max_iteration):
            ...
            for param, (min_val, max_val) in current_ranges.items():
                ...
                if param in ['hidden_neurons', 'epochs']:
                    ...
                    if new_max - new_min < 0.003:
                        new_min = max(best_val - 0.003, min_val)
                        new_max = min(best_val + 0.003, max_val)
    ...
    def _save_results(self):
        ...
        filepath = os.path.join(self.results_dir, f"monk3_dichotomic_{timestamp}.json")
    ...
    def run_trials(self, X_train, y_train, X_test, y_test, n_trials=1000, 
        ...
        print("\n" + "="*60)
        print(f"ESECUZIONE DI {n_trials} TRIAL - MONK3")
        print("="*60)
